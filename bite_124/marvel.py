...
def most_popular_characters(characters=characters, top=5):
    ...
    for char in characters:
        try:
            most_popular[char.name + "^" + char.year] += int(char.appearances)
        except ValueError:
            most_popular[char.name + "^" + char.year] += 0

    return [x[0].split("^")[0] for x in most_popular.most_common(top)]
...
def max_and_min_years_new_characters(characters=characters):
    ...
    for char in characters:
        if char.year != '':
            introduced[char.year] += 1

    return (introduced.most_common()[0][0], introduced.most_common()[-1][0])
...
def get_percentage_female_characters(characters=characters):
    ...
    female = sex['Female Characters'] / (
        sum([sex[x] for x in sex])
    )

    return round(female * 100, 2)
