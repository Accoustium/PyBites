    ...
    for character in characters:
        if type(character) != str:
            for char in character:
                for line, data in enumerate(text):
                    if char.lower() in data.lower():
                        character_index[character[0].lower()].append(line)
        else:
            for line, data in enumerate(text):
                if character.lower() in data.lower():
                    character_index[character.lower()].append(line)

    for key, value in character_index.items():
        character_index.update({ key : sorted(list(set(value))) })
