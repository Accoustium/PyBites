...
def diehard_pybites():
    ...
    """
    for user in gen_files():
        use = user.split("/")
        users[use[1]] += 1
        popular_challenges[use[0]] += 1

    for user in users.most_common():
        if user[0] in IGNORE:
            pass
        else:
            stats = Stats(user[0], popular_challenges.most_common(1)[0])
            break
